...
inline std::string notation(const Move &move, const char *pieceLetter,
                            bool capture, bool check, bool mate, bool promotion,
                            PieceType promoType) {
  std::string s;
  if (move.isCastling) {
...
inline std::string specialEvent(const MoveAnnotation &ann) {
  ...
  if (t.checkmate)
    return "GAME END: CHECKMATE - " + mover + " delivers checkmate with " +
           ann.san + ".";
  if (t.stalemate) {
    if (ann.impact == ImpactLevel::Critical || ann.winChanceBefore >= 0.70f)
      return "CRITICAL BLUNDER: STALEMATE - " + mover +
             " accidentally stalemates " + enemy + " with " + ann.san +
             ". Victory thrown away into a draw!";
    return "GAME END: STALEMATE - " + mover + " stalemates " + enemy +
           " with " + ann.san + ". The match ends in a draw.";
  }
  if (t.draw)
    return "GAME END: DRAW - The game concludes in a draw by rule.";

  if (ann.quality == MoveQuality::Brilliant)
    return "CRITICAL EVENT: BRILLIANT MOVE - " + mover +
           " executes a deep tactical strike with " + ann.san + "!";

  if (ann.quality == MoveQuality::Blunder) {
    if (ann.impact == ImpactLevel::Critical) {
      return "CRITICAL BLUNDER: Catastrophic disaster! " + mover +
             " blunders with " + ann.san +
             ", collapsing win probability and throwing away the match.";
    }
    if (ann.impact == ImpactLevel::Major) {
      return "MAJOR BLUNDER: Heavy setback. " + mover +
             " commits a serious blunder with " + ann.san +
  ...
  if (ann.quality == MoveQuality::Mistake) {
    if (ann.impact >= ImpactLevel::Major) {
      return "MAJOR MISTAKE: Significant error. " + mover + " plays " +
             ann.san + ", conceding valuable ground.";
    }
    return "MINOR MISTAKE: " + mover + " plays " + ann.san +
           " with a minor positional concession.";
  }

  if (t.fork)
    return imp + " TACTIC: FORK - " + mover + " " + pieceName + " on " +
           ann.move.to.algebraic() +
           " attacks multiple enemy pieces simultaneously.";
  if (t.pin)
    return imp + " TACTIC: PIN - " + mover + " " + pieceName + " on " +
           ann.move.to.algebraic() + " pins an enemy piece against the King.";
  if (t.skewer)
    return imp + " TACTIC: SKEWER - " + mover + " " + pieceName + " on " +
           ann.move.to.algebraic() +
  ...
  if (t.doubleCheck)
    return imp + " TACTIC: DOUBLE CHECK - " + mover +
           " attacks the King with two pieces simultaneously.";
  if (t.discoveredAttack)
    return imp + " TACTIC: DISCOVERED ATTACK - " + mover +
           " uncovers an attack on an enemy target.";
  if (t.hangsPiece)
    return imp + " TACTIC: HANGING PIECE - " + mover + " left " + pieceName +
           " on " + ann.move.to.algebraic() + " undefended.";

  return "";
}

// Trade / Exchange evaluation message (trading blows, favorable/unfavorable
// trades, queen trades, forced trades).
inline std::string tradeEvent(const MoveAnnotation &ann) {
  ...
  if (ann.isRecapture) {
    if (ann.impact == ImpactLevel::Critical)
      return "CRITICAL EXCHANGE: Trading blows! " + mover +
             " executes a game-deciding recapture on " +
             ann.move.to.algebraic() + " with " + ann.san + ".";
    if (ann.impact == ImpactLevel::Major)
      return "MAJOR EXCHANGE: Trading blows! " + mover +
             " immediately recaptures on " + ann.move.to.algebraic() +
             " with " + ann.san + ".";
    return "MINOR EXCHANGE: " + mover + " recaptures on " +
           ann.move.to.algebraic() + " with " + ann.san + ".";
  }

  // Equal material trade
  if (ann.impact == ImpactLevel::Critical)
    return "CRITICAL TRADE: High-stakes trade! " + mover + " trades " +
           pieceName + " for " + capturedName + " on " +
           ann.move.to.algebraic() + ", sealing the advantage.";
  if (ann.impact == ImpactLevel::Major)
    return "MAJOR TRADE: Strategic piece trade. " + mover + " trades " +
           pieceName + " for " + capturedName + " on " +
           ann.move.to.algebraic() + ".";

  return "MINOR TRADE: Routine piece trade. " + mover + " exchanges " +
         pieceName + " for " + capturedName + " on " + ann.move.to.algebraic() +
         ".";
}
...
inline std::string gameStatus(const MoveAnnotation &ann) {
  ...
  bool moverWasLeading = ann.evalBeforeCp > 120;
  bool enemyWasLeading = ann.evalBeforeCp < -120;
  bool moverNowLeading = ann.evalAfterCp > 120;
  bool enemyNowLeading = ann.evalAfterCp < -120;

  if (enemyWasLeading && moverNowLeading)
    return "MOMENTUM SHIFT: The tables have turned. " + enemy +
           " held the lead, but " + mover + " now seizes full control.";
  if (moverWasLeading && enemyNowLeading)
    return "MOMENTUM SHIFT: The tables have turned. " + mover +
           " held the lead, but " + enemy + " now seizes full control.";

  // 2. Periodic evaluation every 4 full moves (at the end of Black turn)
  if (ann.fullMoveNumber >= 4 && ann.fullMoveNumber % 4 == 0 &&
...
inline std::string formatLLMEvent(const MoveAnnotation &ann) {
  ...
  std::string s = "Move " + std::to_string(ann.fullMoveNumber) + " (" + mover +
                  "): " + ann.san + ". ";

  if (ann.move.isCastling) {
    s += (ann.move.to.file > ann.move.from.file) ? "Castles kingside. "
                                                 : "Castles queenside. ";
  } else {
    s += std::string(pieceTypeName(ann.pieceMoved)) + " " +
         ann.move.from.algebraic() + " to " + ann.move.to.algebraic() + ". ";
    if (ann.hasCapture) {
      s += "Captures " + enemy + " " +
           std::string(pieceTypeName(ann.pieceCaptured)) + ". ";
    }
  ...
  char winBuf[64];
  std::snprintf(winBuf, sizeof(winBuf), "Win: %s %d - %s %d (Shift: %+d, %s).",
                mover.c_str(), moverWinPct, enemy.c_str(), enemyWinPct,
                shiftPct, impactLevelName(ann.impact));
  s += winBuf;

  if (!ann.specialEvent.empty())
    s += " " + ann.specialEvent;

  if (!ann.tradeEvent.empty())
    s += " " + ann.tradeEvent;

  if (!ann.gameStatus.empty())
    s += " " + ann.gameStatus;
...
inline void decorate(MoveAnnotation &ann, const char *pieceLetter) {
  ann.san = notation(ann.move, pieceLetter, ann.tactics.capture,
                     ann.tactics.check, ann.tactics.checkmate,
...
inline std::string evalText(const MoveAnnotation &ann) {
  ...
  char buf[96];
  std::snprintf(buf, sizeof(buf), "eval %s -> %s, win %d to %d pct (%s)",
                beforeStr.c_str(), afterStr.c_str(), winBeforePct, winAfterPct,
                impactLevelName(ann.impact));
  return buf;
}
} // namespace AnnotationWriter
} // namespace wchess
