...
enum class ImpactLevel : uint8_t { Minor, Major, Critical };
...
inline float winProbability(int centipawns) {
  // P(win) = 1.0 / (1.0 + 10^(-cp / 400.0))
  float exponent = -static_cast<float>(centipawns) / 400.0f;
...
struct Square {
  ...
  constexpr int index() const { return rank * 8 + file; }
...
struct Move {
  ...
  constexpr bool operator==(const Move &other) const {
    return from == other.from && to == other.to &&
           (isPromotion ? promotion == other.promotion : true);
  }
};
...
struct Eval {
  bool valid = false;
  int centipawns =
      0; // positive = good for the side to move in the evaluated position
...
struct MoveAnnotation {
  ...
  std::string
      specialEvent; // Special tactical highlight (forks, pins, blunders, etc.)
