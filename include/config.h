...
inline constexpr float BOARD_RIGHT_MARGIN_PX =
    24.0f; // board right edge -> screen centre distance
inline constexpr float BOARD_MAX_HEIGHT_RATIO =
    0.82f; // board may occupy up to 82% of the window height
inline constexpr float BOARD_MAX_WIDTH_RATIO =
    0.92f; // board may occupy up to 92% of the left half width
inline constexpr float RANK_LABEL_GAP_PX =
    10.0f; // rank labels sit this many px left of the board
inline constexpr float FILE_LABEL_GAP_PX =
    8.0f; // file labels sit this many px below the board

// ---- Story panel (screen px) ----
inline constexpr float PANEL_LEFT_MARGIN_PX =
    40.0f; // story text starts this far right of the centre
...
inline const std::vector<std::string> STOCKFISH_CANDIDATES = {
    "bin/stockfish", "../bin/stockfish"};
...
inline constexpr int AI_SEARCH_DEPTH =
    1; // shallow search depth (1-ply) for easy play
inline constexpr float AI_BLUNDER_CHANCE =
    0.35f;                                // 35% chance of casual random moves
inline constexpr int AI_MOVETIME_MS = 50; // max think time for AI reply
inline constexpr int EVAL_MOVETIME_MS =
    350; // think time used for move classification / narrator
...
inline constexpr float MISS_WIN_PAWNS =
    2.0f; // a capture win this big that was ignored = "miss"
