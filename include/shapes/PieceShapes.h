	...
	namespace PieceShapes
	{
		...
		namespace detail
		{
			...
			struct Builder
			{
				...
				{
					return std::make_shared<Addition>(x, std::make_shared<Multiplication>(s, ox));
				}

				std::shared_ptr<IMathExpression> py(float oy) const
				{
					return std::make_shared<Addition>(y, std::make_shared<Multiplication>(s, oy));
				}
				...
				std::shared_ptr<IMathExpression> box(float ox, float oy, float w, float h) const
				{
					return std::make_shared<Primitives::Box>(px(ox), py(oy), scaled(w), scaled(h));
				}
				...
				std::shared_ptr<IMathExpression> line(float ax, float ay, float bx, float by, float width) const
				{
					return std::make_shared<Primitives::Line>(px(ax), py(ay), px(bx), py(by), scaled(width));
				}

				std::shared_ptr<IMathExpression> triangle(float ox, float oy, float w, float h) const
				{
					return std::make_shared<Primitives::Triangle>(px(ox), py(oy), scaled(w), scaled(h),
																  std::make_shared<FloatConstant>(0.0f));
				}

				// union of two shapes with a soft seam
				std::shared_ptr<IMathExpression> softUnion(std::shared_ptr<IMathExpression> a,
														   std::shared_ptr<IMathExpression> b) const
				{
					...
					return std::make_shared<SDFSmoothAddition>(std::move(a), std::move(b), scaled(0.03f));
				}

				// hollow outline of a shape
				std::shared_ptr<IMathExpression> onion(std::shared_ptr<IMathExpression> a, float thickness) const
				{
		...
		inline void registerAll(ShapeService& shapes)
		{
			...
			{
				auto shape = b.softUnion(makeBase(), b.triangle(0.0f, -0.06f, 0.28f, 0.26f));
			...
			{
				auto shape = b.softUnion(makeBase(), b.triangle(-0.02f, 0.06f, 0.40f, 0.48f));
			...
			{
				auto shape = b.softUnion(makeBase(), b.triangle(0.0f, -0.02f, 0.30f, 0.36f));
