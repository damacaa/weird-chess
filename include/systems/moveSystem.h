...
inline void syncPieces(ChessState &state, Registry &registry,
                       ShapeService &shapes) {
  ...
  for (int index = 0; index < 64; ++index) {
    ...
    Entity entity = PieceShapes::spawnPiece(
        registry, shapes, piece->first, piece->second, c.x, c.y, PIECE_SCALE);
    ...
    if (registry.hasComponent<SquareComp>(state.squareEntities[index]))
      registry.getComponent<SquareComp>(state.squareEntities[index]).piece =
          entity;
  }
}
...
inline void animatePiece(ChessState &state, Registry &registry, int fromIndex,
                         int toIndex) {
...
inline void refreshHighlights(ChessState &state, Registry &registry,
  ...
  if (state.selectedSquare >= 0)
    BoardShapes::setHighlight(
        registry, state.highlightEntities[state.selectedSquare],
        state.selectedSquare, ChessPalette::HighlightGreen);
  ...
  if (state.hasLastMove) {
    ...
    if (fromIdx != state.selectedSquare)
      BoardShapes::setHighlight(registry, state.highlightEntities[fromIdx],
                                fromIdx, ChessPalette::HighlightYellow);
  ...
  if (!state.gameOver && state.board->inCheck()) {
    ...
    BoardShapes::setHighlight(registry, state.highlightEntities[kingIdx],
                              kingIdx, ChessPalette::CheckRed);
  }
...
inline void applyMove(ChessState &state, Registry &registry,
                      ServiceProvider &services, const Move &move,
                      Color mover) {
...
inline void resetGame(ChessState &state, Registry &registry,
                      ServiceProvider &services) {
