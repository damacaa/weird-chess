...
inline std::vector<std::string> wrapLines(const std::string &text,
  ...
  auto pushWrapped = [&](const std::string &line) {
    ...
    while (start < line.size()) {
      size_t end =
          std::min(line.size(), start + static_cast<size_t>(wrapChars));
  ...
  if (state.statusText != INVALID_ENTITY) {
    ...
    if (state.gameOver) {
      ...
      case GameState::Checkmate:
        status = state.board->sideToMove() == Color::White
      ...
      status = state.board->sideToMove() == Color::White ? "WHITE TO MOVE"
                                                         : "BLACK TO MOVE";
    } else {
      status = state.board->sideToMove() ==
