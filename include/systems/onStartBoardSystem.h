...
inline void onStartBoardSystem(Registry &registry, ServiceProvider &services) {
  ...
  // Stockfish if available, otherwise the built-in fallback AI.
  std::string sfPath;
  ...
  if (!sfPath.empty()) {
    ...
    if (stockfish->isAvailable()) {
      stockfish->setStrength(ChessConfig::DEFAULT_SKILL,
                             ChessConfig::DEFAULT_ELO);
      state.ai = stockfish;
    }
  }
  if (!state.ai) {
    std::cout << "[WeirdChess] Stockfish not found, using NullAI (random "
                 "moves, material eval)."
              << std::endl;
  ...
  // ---- board ----
  state.squareEntities = BoardShapes::createBoard(registry, services.shapes(),
                                                  state.highlightEntities);
  ...
  state.titleText = UIButtonFactory::createText(
      registry, 0.0f, 0.0f, "WEIRD CHESS", ChessPalette::PanelTitle);

  for (int i = 0; i < ChessConfig::STORY_MAX_LINES; ++i) {
    state.storyLines.push_back(UIButtonFactory::createText(
        registry, 0.0f, 0.0f, "", ChessPalette::PanelText));
  }
