...
static void testCastling() {
  ...
  std::string sanK = AnnotationWriter::notation(castleK, "K", false, false,
                                                false, false, PieceType::Queen);
  CHECK(sanK == "O-O");

  CHECK(board.makeMove(castleK));
  auto kingPiece = board.pieceAt(Square{6, 0});
  auto rookPiece = board.pieceAt(Square{5, 0});
  CHECK(kingPiece && kingPiece->second == PieceType::King &&
        kingPiece->first == Color::White);
  CHECK(rookPiece && rookPiece->second == PieceType::Rook &&
        rookPiece->first == Color::White);
  ...
  std::string bSanK = AnnotationWriter::notation(
      bCastleK, "K", false, false, false, false, PieceType::Queen);
  CHECK(bSanK == "O-O");
  CHECK(bBoard.makeMove(bCastleK));

  auto bKing = bBoard.pieceAt(Square{6, 7});
  auto bRook = bBoard.pieceAt(Square{5, 7});
  CHECK(bKing && bKing->second == PieceType::King &&
        bKing->first == Color::Black);
  CHECK(bRook && bRook->second == PieceType::Rook &&
        bRook->first == Color::Black);
  ...
  std::string bSanQ = AnnotationWriter::notation(
      bCastleQ, "K", false, false, false, false, PieceType::Queen);
  CHECK(bSanQ == "O-O-O");
  CHECK(bBoard.makeMove(bCastleQ));

  auto bKingQ = bBoard.pieceAt(Square{2, 7});
  auto bRookQ = bBoard.pieceAt(Square{3, 7});
  CHECK(bKingQ && bKingQ->second == PieceType::King &&
        bKingQ->first == Color::Black);
  CHECK(bRookQ && bRookQ->second == PieceType::Rook &&
        bRookQ->first == Color::Black);
  ...
  std::string sanQ = AnnotationWriter::notation(castleQ, "K", false, false,
                                                false, false, PieceType::Queen);
  CHECK(sanQ == "O-O-O");
  ...
  CHECK(kingQ && kingQ->second == PieceType::King &&
        kingQ->first == Color::White);
  CHECK(rookQ && rookQ->second == PieceType::Rook &&
        rookQ->first == Color::White);
  CHECK(!board.pieceAt(Square{4, 0}));
  ...
  CHECK(board.pieceAt(Square{6, 0}) &&
        board.pieceAt(Square{6, 0})->second == PieceType::King);
  CHECK(board.pieceAt(Square{5, 0}) &&
        board.pieceAt(Square{5, 0})->second == PieceType::Rook);
...
static void testEnPassant() {
  ...
  std::string san = AnnotationWriter::notation(epMove, "", true, false, false,
                                               false, PieceType::Queen);
  CHECK(san == "exd6");

  CHECK(board.makeMove(epMove));
  auto wPawn = board.pieceAt(Square{3, 5}); // d6
  CHECK(wPawn && wPawn->second == PieceType::Pawn &&
        wPawn->first == Color::White);
  ...
  CHECK(wPawnOrig && wPawnOrig->second == PieceType::Pawn &&
        wPawnOrig->first == Color::White);
  CHECK(bPawnOrig && bPawnOrig->second == PieceType::Pawn &&
        bPawnOrig->first == Color::Black);
  ...
  std::string bSan = AnnotationWriter::notation(bEpMove, "", true, false, false,
                                                false, PieceType::Queen);
  CHECK(bSan == "exd3");

  CHECK(board.makeMove(bEpMove));
  auto bPawn = board.pieceAt(Square{3, 2}); // d3
  CHECK(bPawn && bPawn->second == PieceType::Pawn &&
        bPawn->first == Color::Black);
  CHECK(!board.pieceAt(Square{4, 3})); // e4 is empty
  CHECK(!board.pieceAt(Square{3, 3})); // d4 captured pawn is gone

  board.undoMove();
  CHECK(board.pieceAt(Square{4, 3}) &&
        board.pieceAt(Square{4, 3})->second == PieceType::Pawn);
  CHECK(board.pieceAt(Square{3, 3}) &&
        board.pieceAt(Square{3, 3})->second == PieceType::Pawn);
  ...
  CHECK(board.pieceAt(Square{3, 5}) &&
        board.pieceAt(Square{3, 5})->second == PieceType::Pawn);
...
static void testPromotion() {
  ...
  // Test Annotation
  std::string sanQ = AnnotationWriter::notation(moveQ, "", false, false, false,
                                                true, PieceType::Queen);
  CHECK(sanQ == "e8=Q");
  std::string sanN = AnnotationWriter::notation(moveN, "", false, false, false,
  ...
  CHECK(promotedQueen && promotedQueen->second == PieceType::Queen &&
        promotedQueen->first == Color::White);
  CHECK(!board.pieceAt(Square{4, 6}));

  // Test Undo
  board.undoMove();
  auto restoredPawn = board.pieceAt(Square{4, 6});
  CHECK(restoredPawn && restoredPawn->second == PieceType::Pawn &&
        restoredPawn->first == Color::White);
  ...
  CHECK(promotedKnight && promotedKnight->second == PieceType::Knight &&
        promotedKnight->first == Color::White);
  ...
  for (const auto &m : capPromoMoves) {
    if (m.to == Square{3, 7} && m.isPromotion &&
  ...
  std::string sanCap = AnnotationWriter::notation(
      capPromoQ, "", true, false, false, true, PieceType::Queen);
  CHECK(sanCap == "exd8=Q");
  ...
  CHECK(bQueen && bQueen->second == PieceType::Queen &&
        bQueen->first == Color::Black);
...
int main(int argc, char **argv) {
  ...
  if (!state.ai) {
    printf("AI: NullAI (no stockfish binary at %s)\n", sfPath.c_str());
    state.ai = std::make_shared<NullAI>();
  }
  ...
  while (!state.board->isGameOver() && guard++ < 300) {
    ...
    if (guard <= 8) {
      printf("ply %2d: %s %-8s %-14s loss %5.2f  eval %+.2f\n", guard,
             mover == Color::White ? "W" : "B", ann.san.c_str(),
             qualityName(ann.quality), static_cast<float>(ann.lossCp) / 100.0f,
  ...
  printf("game ended after %d plies: %s\n", guard,
         state.board->isGameOver()
             ? (state.board->gameState() == GameState::Checkmate
                    ? "checkmate"
                    : (state.board->gameState() == GameState::Stalemate
                           ? "stalemate"
                           : "draw"))
             : "not finished (guard)");
  CHECK(annotations > 0);
  ...
  // ---- let the narrator worker finish the last chunk ----
  {
    int spins = 0;
    while (state.narrator->stream()->status() == StoryStatus::Generating &&
           spins++ < 100) {
      using namespace std::chrono_literals;
